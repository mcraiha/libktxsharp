...
/// <summary>
/// Load Ktx (.ktx files) input static class
/// </summary>
public static class KtxLoader
{
	...
	public static (bool isValid, string possibleError) CheckIfInputIsValid(Stream stream)
	{
		...
		(bool isIdentifierValid, string possibleIdentifierError) = KtxValidators.ValidateKtx1Identifier(stream);
		if (!isIdentifierValid)
		{
			return (isValid: false, possibleError: possibleIdentifierError);
		}

		(bool isHeaderValid, string possibleHeaderError) = KtxValidators.ValidateKtx1HeaderData(stream);
		if (!isHeaderValid)
		{
		...
		(bool isTextureDataValid, string possibleTextureDataError) = KtxValidators.ValidateKtx1TextureData(stream, tempHeader, (uint)(stream.Length - stream.Position));
